...
- Write, edit, and bash stay inside the workspace. Read, glob, and
  grep of paths outside the workspace ask the operator; they do not
  go to the Review model. When Skills is enabled, any path inside a
  Skills search directory (`skill` / `skills` trees) auto-passes.
  Credential paths stay Forbidden.
